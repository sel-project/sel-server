...
public enum string[][uint] bedrockSupportedProtocols = [
	100u: ["1.0.0", "1.0.1", "1.0.2"],
	101u: ["1.0.3"],
	102u: ["1.0.4"],
	105u: ["1.0.5"],
	113u: ["1.1.0", "1.1.1", "1.1.2", "1.1.3", "1.1.4"],
...
class BedrockServer(uint[] supportedProtocols=bedrockSupportedProtocols.keys) : GenericServer {
	...
	protected shared void receivePackets(bool ipv4)(shared Socket socket, shared Handler handler, shared Query query) {
		...
		immutable protocolsString = to!string(this.protocols[$-1]) ~ ";" ~ supportedProtocols[this.protocols[$-1]][0];
		static if(ipv4) {
								...
								if(clientData.type == JSON_TYPE.OBJECT) {
									this.client = (){
										final switch(protocol) {
											foreach(p ; TupleOf!(BedrockServer.supportedProtocols.keys)) {
												case p:
